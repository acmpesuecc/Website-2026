...
window.closeNiriWindow = function (win) {
  ...
  const isFirstWindow =
    !win.previousElementSibling ||
    !win.previousElementSibling.classList.contains("niri-window");
  const hasOtherWindows =
    !!win.nextElementSibling &&
    win.nextElementSibling.classList.contains("niri-window");
  ...
  if (
    track.id !== "niri-track-h-root" &&
    track.querySelectorAll(".niri-window").length === 1
  ) {
    removeTrackAndFocusPrev(track);
  } else {
    removeWindowAndFocusBest(win);
...
window.centerAndFocusWindow = function () {
  ...
  windows.forEach((win) => {
    ...
    const distance = Math.sqrt(
      Math.pow(centerX - winCenterX, 2) + Math.pow(centerY - winCenterY, 2),
    );
  ...
  if (
    ...
    !closest.contains(document.activeElement)
  ) {
...
document.addEventListener(
  ...
  (e) => {
    if (
      e.target.id === "niri-track-v" ||
      (e.target.classList &&
        e.target.classList.contains("niri-horizontal-track"))
    ) {
...
function injectWindowControls(container) {
  ...
  wins.forEach((win) => {
    ...
    if (!win.querySelector(".niri-window-controls")) {
      ...
      if (isFull) {
        minBtn.innerHTML =
          '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="4" width="13" height="13" rx="2" ry="2"></rect><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"></path></svg>';
        minBtn.setAttribute("aria-label", "Restore window size");
      } else {
        minBtn.innerHTML =
          '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>';
...
document.addEventListener("fx:after", (e) => {
  ...
  if (content) {
    ...
    if (
      !content.id ||
      content.id === "root-window" ||
      content.id === "win-direct"
    ) {
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll(".niri-window").forEach(wrapWindowContent);
...
function disableAllRibbons() {
  document
    .querySelectorAll(".niri-horizontal-track")
    .forEach((r) => disableRibbonScroll(r));
}
...
try {
  window.__niriDebugOverview = true;
} catch (err) {}
...
function overviewWheelHandler(e) {
  ...
  function findScrollTarget(axis) {
    if (axis === "x") {
      ...
      if (
        ...
        isScrollable(document.scrollingElement, "y")
      )
        return document.scrollingElement;
      if (document.body && isScrollable(document.body, "y"))
  ...
  if (absY >= absX) {
    ...
    if (
      window.__niriDebugOverview ||
      (localStorage && localStorage.debugOverview === "1")
    )
      console.log("[overview] vertical wheel", {
        deltaY,
  ...
  if (
    ...
    console.log("[overview] horizontal wheel", {
      deltaX,
  ...
  try {
    ...
    if (!target.__prevScrollSnap)
      target.__prevScrollSnap = target.style.scrollSnapType || "";
  ...
  window.requestAnimationFrame(() => {
    ...
    if (
      ...
      try {
        console.log("[overview] post-scroll", {
          scrollLeft: target.scrollLeft,
          scrollWidth: target.scrollWidth,
          clientWidth: target.clientWidth,
          amount,
    ...
    target.__snapRestoreTimeout = setTimeout(() => {
      try {
        target.style.scrollSnapType = target.__prevScrollSnap || "";
        delete target.__prevScrollSnap;
      } catch (e) {}
    }, 700);
  });
}
...
function overviewTouchMove(e) {
  if (!document.body.classList.contains("overview-mode") || !_touchStart)
    return;
  const t = e.touches && e.touches[0];
  if (!t) return;
  ...
  if (absY >= absX) {
    ...
    if (root) {
      ...
      _touchStart.x = t.clientX;
      _touchStart.y = t.clientY;
    }
  } else {
  ...
  (e) => {
    if (document.body.classList.contains("overview-mode")) {
      ...
      if (t) {
        ...
        const ribbon =
          el && el.closest ? el.closest(".niri-horizontal-track") : null;
        if (ribbon) enableRibbonScroll(ribbon);
      }
    }
...
document.addEventListener("touchmove", overviewTouchMove, {
  passive: false,
  capture: true,
});
...
function overviewKeyBlocker(e) {
  ...
  switch (e.key) {
    case "ArrowDown":
      _scrollTrackBy(root, 0, Math.max(80, Math.round(vw * 0.08)), true);
      break;
    case "ArrowUp":
      _scrollTrackBy(root, 0, -Math.max(80, Math.round(vw * 0.08)), true);
      break;
    ...
    case "Home":
      _scrollTrackBy(
        root,
        0,
        -document.scrollingElement.scrollTop || -9999999,
        true,
      );
      break;
    case "End":
      _scrollTrackBy(
        root,
        0,
        document.scrollingElement.scrollHeight || 9999999,
        true,
      );
      break;
    case "ArrowLeft":
      _scrollTrackBy(ribbon || root, -Math.round(hx * 0.2), 0, true);
      break;
    case "ArrowRight":
      _scrollTrackBy(ribbon || root, Math.round(hx * 0.2), 0, true);
      break;
